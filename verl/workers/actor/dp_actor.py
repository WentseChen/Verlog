...
def compute_off_policy_metrics(log_prob, old_log_prob, advantages, response_mask, clip_ratio_low, clip_ratio_high, deltas):
    ...
    return {
        ...
        "actor/IS_delta": E_is_delta.detach().item(),
        "actor/IS_delta_std": std_is_delta.detach().item(),
        "actor/IS_delta_clipped": E_is_delta_clipped.detach().item(),
        "actor/IS_delta_clipped_std": std_is_delta_clipped.detach().item(),
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        self, micro_batch, temperature, calculate_entropy=False, return_all_logits=False
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False, return_all_logits=False) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            ...
            with torch.no_grad():
                entropy, log_probs = self._forward_micro_batch(
                    model_inputs, temperature=temperature, calculate_entropy=calculate_entropy, return_all_logits=return_all_logits,
                )
