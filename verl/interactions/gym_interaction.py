...
class GymInteraction(BaseInteraction):
    """A demo interaction for calculating the reward of gym.
    ...
    async def generate_response(
        self, env, counter, instance_id: str, messages: list[dict[str, Any]], **kwargs
    ) -> tuple[bool, str, float, dict]:
            
        last_msg = messages[-1]["content"]
        response, reward, terminated, truncated, info = env.step(actions)
        done = np.logical_or(terminated, truncated) 

        return done, response, reward, {}
