...
class AsyncTickerAdmissionsEnv(gym.Env):
    ...
    def build_system_prompt(self, professor_id: str | None, preference_vector: List[int] | None = None) -> str:
        ...
        return (
            ...
            f"- Plain text or responses missing <THINK> will be treated as discuss-type actions.\n"
            f"- Never add text outside the tags.\n\n"
            f"- Keep <GROUP> messages concise - aim for 3 sentences or less.\n"
            f"- Keep <THINK> reasoning concise - aim for 5 sentences or less to organize your thoughts efficiently.\n"
            f"STRATEGIC CONSIDERATIONS:\n"
            ...
            f"- Use <WAIT_FOR> to save token budget or to wait for additional input from other professors.\n"
...
class AsyncTickerEnvWrapper(gym.Wrapper):
    """
    Wrapper around AsyncTickerAdmissionsEnv compatible with tool_agent_loop.py.

    Normalizes the multi-agent dict outputs of the inner env to the scalar
    interface expected by ToolAgentLoop.run():
      - reset(agent_id=None) -> (messages, info)
      - step(action)         -> (messages, reward, terminated, truncated, info)
      - get_last_obs(agent_id=None) -> (messages, info) or (None, None)

    The returned `info` dict is the active agent's info with an added
    "agent_id" key so that tool_agent_loop can do info.get("agent_id").
    """
    ...
    def reset(self, agent_id: str | None = None) -> Tuple[List[Dict[str, str]], Dict]:
        observations, infos = self.env.reset()
        active_agent = self.env.episode_state["active_agent"]
        info = deepcopy(infos[active_agent])
        info["agent_id"] = active_agent
        self._last_observations = observations
    ...
    def step(
        self, action: str | Dict[str, Any]
    ) -> Tuple[List[Dict[str, str]], float, bool, bool, Dict]:
        observations, rewards, terminations, truncations, infos = self.env.step(action)
        active_agent = self.env.episode_state["active_agent"]
        reward = rewards[active_agent]
        terminated = terminations[active_agent]
        truncated = truncations[active_agent]
        info = deepcopy(infos[active_agent])
        info["agent_id"] = active_agent
        self._last_observations = observations
        self._last_info = info
    ...
    def get_last_obs(
        ...
        """Return the last (observations, info) from the most recent reset() or step()."""
        if self._last_observations is None or self._last_info is None:
            return None, None
        return self._last_observations, self._last_info